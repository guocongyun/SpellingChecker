...
class Battle:
    def generating_statistic(self):
        ...
        for number in range(game_system.life):
            game_system.life_identifier.append(
                canvas.tag_raise(canvas.create_image(h + number * 30, h,
                                                     image=self.life,
                                                     anchor=NW)))
    ...
    def enemy_creation(self):
        ...
        while enemy_count < number_of_enemy:
            ...
            if not overlap:
                ...
                if enemy.color == "":
                    enemy.identifier = canvas.create_rectangle(
                        enemy.xy, width=3)
            ...
            if game_system.pause or self.ability == "stop":
                if game_system.character[enemy_number].color != "" and \
                    ...
                    canvas.itemconfig(
                        ...
                        fill="grey")
            if not game_system.pause and self.ability != "stop" and \
                    game_system.character[enemy_number].color != "" and \
                    game_system.character[enemy_number].color != "Trap":
                canvas.itemconfig(
                    game_system.character[enemy_number].identifier,
        ...
        for character_i in range(len(game_system.character) - 1):
            ...
            if position_i[0] < position[2] and position_i[2] > \
                ...
                if game_system.character[character_i].character_size < \
                        ...
                        game_system.cheat:
                    canvas.delete(game_system.character[
    ...
    def character_movement(self):
        if game_system.game_running:
            ...
            canvas.after(game_system.character[0].character_attribute[4],
                         self.character_movement)
...
class GameSystem:
    ...
    def pausing(self):
        ...
        if self.pause_count == 3:
            ...
            if game_system.pause:
                self.pause_text = canvas.create_text(
                    6 * w, 6 * h,
                    text="PAUSED",
        ...
        if 0 <= self.pause_count < 3:
            self.pause_text = canvas.create_text(6 * w, 6 * h, text=str(
    ...
    def check_game_status(self):
        ...
        if self.win or self.lose or self.you_win != 0:
            ...
            if self.lose:
                ...
                if self.you_lost == 0:
                    self.final_message = canvas.create_text(
                        6 * w, 15 * h,
                        text="YOU LOST",
                        fill="white",
    ...
    def menu(self):
        ...
        self.identifier.append(
            canvas.create_text(4.5 * w, 9 * h, text="LEADER",
                               font="Times 20 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_text(7.5 * w, 9 * h, text="BOARD",
                               font="Times 20 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_text(4.5 * w, 14 * h, text="First place",
                               font="Times 15 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_text(7.5 * w, 14 * h, text="Second place",
                               font="Times 15 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_text(4.5 * w, 20 * h, text="Third place",
                               font="Times 15 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_text(7.5 * w, 20 * h, text="Forth place",
                               font="Times 15 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_text(4.5 * w, 15.5 * h,
                               text=str(user_name[-1]) + " " + str(
                                   user_score[-1]),
                               font="Times 13 italic", fill="white"))
        self.identifier.append(
            canvas.create_text(7.5 * w, 15.5 * h,
                               text=str(user_name[-2]) + " " + str(
                                   user_score[-2]),
                               font="Times 13 italic", fill="white"))
        self.identifier.append(
            canvas.create_text(4.5 * w, 21.5 * h,
                               text=str(user_name[-3]) + " " + str(
                                   user_score[-3]),
                               font="Times 13 italic", fill="white"))
        self.identifier.append(
            canvas.create_text(7.5 * w, 21.5 * h,
                               text=str(user_name[-4]) + " " + str(
                                   user_score[-4]),
                               font="Times 13 italic", fill="white"))
        self.identifier.append(
            canvas.create_text(4.5 * w, 26 * h, text="And you are",
                               font="Times 15 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_window(4.5 * w, 27 * h, window=self.entry,
                                 height=0.75 * h, width=2 * w))
        self.identifier.append(
            canvas.create_window(4.5 * w, 28 * h, window=new_game,
                                 width=1.5 * w, height=0.75 * h))

        self.identifier.append(
            canvas.create_text(7.5 * w, 26 * h, text="Or you were",
                               font="Times 15 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_text(7.5 * w, 27 * h, text=str(character[4]),
                               font="Times 13 bold italic", fill="white"))
        self.identifier.append(
            canvas.create_window(7.5 * w, 28 * h, window=saved_game,
                                 width=1.5 * w, height=0.75 * h))
